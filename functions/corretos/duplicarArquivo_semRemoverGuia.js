...
async function removeExcludedSubjects(auth, documentId, laudo) {
    ...
    while ((match = subjectRegex.exec(text)) !== null) {
        ...
        if (subject && !subject.include) {
            ...
            // Ajuste para garantir que a linha em branco após o subject seja removida
            const adjustedEndIndex = text[endIndex] === '\n' ? endIndex + 1 : endIndex;
...
async function replaceQuestionsWithAnswers(auth, documentId, laudo) {
    const docs = google.docs({ version: 'v1', auth });
    ...
    // Substituir cada question pelo respectivo answer
    const requests = [];
    laudo.sections.forEach(section => {
        section.questions.forEach(question => {
            const regex = new RegExp(question.placeholder, 'g');
            let match;
    ...
    try {
        if (requests.length > 0) {
            ...
            console.log('Questions substituídas por answers no documento:', documentId);
        } else {
            console.log('Nenhuma question substituída.');
        }
    } catch (err) {
        console.error('Erro ao substituir questions no documento:', err);
        throw err;
    }
}
...
const duplicateDocHandler = async (req, res) => {
    ...
    try {
        ...
        await replaceQuestionsWithAnswers(auth, newFileId, laudo);

        console.log('Documento duplicado, subjects excluídos e questions substituídas com sucesso!');
        return res.send({ message: "Documento duplicado, subjects excluídos e questions substituídas com sucesso.", newDocumentId: newFileId, url: `https://docs.google.com/document/d/${newFileId}` });
    } catch (error) {
