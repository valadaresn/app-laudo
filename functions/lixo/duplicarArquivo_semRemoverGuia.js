...
async function appendLaudoToDoc(auth, documentId, laudo) {
    ...
    const requests = laudo.sections.map(section => {
        const content = [
            {
                insertText: {
                    location: {
    ...
    try {
        ...
        console.log('Laudo adicionado ao documento:', documentId);
    } catch (err) {
        console.error('Erro ao adicionar laudo ao documento:', err);
        throw err;
    }
}
...
const duplicateDocHandler = async (req, res) => {
    ...
    try {
        ...
        console.log('Documento duplicado e laudo adicionado com sucesso!');
        return res.send({ message: "Documento duplicado e laudo adicionado com sucesso.", newDocumentId: newFileId, url: `https://docs.google.com/document/d/${newFileId}` });
    } catch (error) {
